...
from operation_service.utils.utility_functions import get_str_datetime
...
class JSONSerializer():
    ...
    def handle_field(self, mod, field):
        ...
        elif field.get_internal_type() in self.datetime_fields:
            self.stream.write(u': ')
            self.handle_simple(get_str_datetime(field))
        else:
            self.stream.write(u': ')
    ...
    def handle_fk_field(self, mod, field):
        ...
        if related is not None:
            ...
            if type(d['id']) == str and d['id'].isdigit():
